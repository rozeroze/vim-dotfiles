...
function! dungeon#dungeon#turnlist.next(loop)
   call New('turnlist next(' . a:loop . ') start')
   if a:loop == 0 | return 0 | endif
   call New('let who = first()')
   let who = g:dungeon#dungeon#turnlist.first()
   call New('who is ' . who.name)
   call New('dropout(0)')
   call g:dungeon#dungeon#turnlist.dropout(0)
   call New('append(' . who.name . ')')
   call g:dungeon#dungeon#turnlist.append(who)
   call New('next(loop - 1)')
   call g:dungeon#dungeon#turnlist.next(a:loop - 1)
   call New('turnlist next() end')
endfunction
" }}}
...
" }}}
" dungeon#dungeon#util.getdirectionname(direction) {{{
function dungeon#dungeon#util.getdirectionname(direction)
   if a:direction == 0 || a:direction == 'k'
      return 'up'
   endif
   if a:direction == 1 || a:direction == 'l'
      return 'right'
   endif
   if a:direction == 2 || a:direction == 'j'
      return 'down'
   endif
   if a:direction == 3 || a:direction == 'h'
      return 'left'
   endif
endfunction
" }}}
...
function dungeon#dungeon#default.action() dict
   call g:dungeon#dungeon#history.new(self.name . 's action')
   if !self.attack()
      call self.move()
   endif
endfunction
...
function dungeon#dungeon#default.attack() dict
   call g:dungeon#dungeon#history.new(self.name . ' attack')
   "return 1
   return 0
endfunction
" }}}
...
" }}}
" dungeon#dungeon#default.aim(who) {{{
function! dungeon#dungeon#default.aim(who)
   if a:who.direction == 'h' || a:who.direction == 'left'
      return { 'x': a:who.x - 1, 'y': a:who.y }
   endif
   if a:who.direction == 'j' || a:who.direction == 'down'
      return { 'x': a:who.x, 'y': a:who.y + 1 }
   endif
   if a:who.direction == 'k' || a:who.direction == 'up'
      return { 'x': a:who.x, 'y': a:who.y - 1 }
   endif
   if a:who.direction == 'l' || a:who.direction == 'right'
      return { 'x': a:who.x + 1, 'y': a:who.y }
   endif
   return v:false
endfunction
" }}}
" dungeon#dungeon#default.move() dict {{{
function dungeon#dungeon#default.move() dict
   ...
   call New(self.name . ' moved')
   return 0

   let go = g:dungeon#dungeon#default.aim(self)
   let movable = g:dungeon#dungeon#field.isfloor(go.x, go.y)
   if !movable
      return v:false
   endif
   let dirname = g:dungeon#dungeon#util.getdirectionname(self.direction)
   if has_key(self, 'move_' . dirname)
      call self['move_' . dirname]()
   else
      call g:dungeon#dungeon#default['move_' . dirname](self)
   endif
endfunction
...
" }}}
" dungeon#dungeon#keeper.action() {{{
function! dungeon#dungeon#keeper.action()
   call New('keeper action()')
   let c = nr2char(getchar())
   if c == 'q' || c == ''
      let g:dungeon#dungeon#event.loop = v:false
   endif
   call g:dungeon#dungeon#history.new('keeper action: ' . c)
   if c == 'h' || c == 'j' || c == 'k' || c == 'l'
      let g:dungeon#dungeon#keeper.direction = c
      let moved = g:dungeon#dungeon#keeper.move()
   endif
endfunction
" }}}
...
function dungeon#dungeon#keeper.move()
   call New('keeper.move()')
   let go = g:dungeon#dungeon#default.aim(g:dungeon#dungeon#keeper)
   let movable = g:dungeon#dungeon#field.isfloor(go.x, go.y)
   if !movable
      return v:false
   endif
   call g:dungeon#dungeon#keeper['move_' . g:dungeon#dungeon#keeper.direction]()
endfunction
...
function New(text)
   " easy history
   call g:dungeon#dungeon#history.new(a:text)
endfunction
" }}}
...
" }}}
" dungeon#dungeon#history.show() {{{
function dungeon#dungeon#history.show()
   " MEMO: test
   let all = deepcopy(self.histories)
   vnew all-history
   setlocal buftype=nofile
   call append(0, all)
endfunction
...
let g:dungeon#dungeon#config = {}
let g:dungeon#dungeon#config.sleeptime = 10
...
function dungeon#dungeon#event.start()
   let g:dungeon#dungeon#event.loop = v:true
   while g:dungeon#dungeon#event.loop
      let order = nr2char(getchar(0))
      if order == 'q' | let g:dungeon#dungeon#event.loop = v:false | endif
